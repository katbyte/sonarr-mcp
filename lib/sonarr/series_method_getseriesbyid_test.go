...
func TestOperationGetSeriesById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSeriesById(t.Context(), 7, GetSeriesByIdOperationOptions{
		IncludeSeasonImages: new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/series/7")
	expectQuery(t, r, "includeSeasonImages", "true")
