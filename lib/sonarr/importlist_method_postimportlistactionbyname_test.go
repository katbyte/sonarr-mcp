...
func TestOperationPostImportListActionByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostImportListActionByName(t.Context(), "p/name", ImportListResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v3/importlist/action/p%2Fname")
