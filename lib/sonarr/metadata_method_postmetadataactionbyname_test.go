...
func TestOperationPostMetadataActionByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostMetadataActionByName(t.Context(), "p/name", MetadataResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v3/metadata/action/p%2Fname")
