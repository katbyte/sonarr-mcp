...
func TestOperationGetQueueDetails(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetQueueDetails(t.Context(), GetQueueDetailsOperationOptions{
		SeriesId:       7,
		EpisodeIds:     []int{1, 2},
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/queue/details")
