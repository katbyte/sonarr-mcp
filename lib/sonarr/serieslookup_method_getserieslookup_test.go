...
func TestOperationGetSeriesLookup(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetSeriesLookup(t.Context(), GetSeriesLookupOperationOptions{
		Term: "v-Term",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/series/lookup")
	expectQuery(t, r, "term", "v-Term")
