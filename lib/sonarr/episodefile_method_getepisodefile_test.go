...
func TestOperationGetEpisodeFile(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetEpisodeFile(t.Context(), GetEpisodeFileOperationOptions{
		SeriesId:       7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v3/episodefile")
	expectQuery(t, r, "seriesId", "7")
	expectQuery(t, r, "episodeFileIds", "1", "2")
