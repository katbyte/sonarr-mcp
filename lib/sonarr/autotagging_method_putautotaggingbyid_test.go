...
func TestOperationPutAutoTaggingById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutAutoTaggingById(t.Context(), "p/id", AutoTaggingResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v3/autotagging/p%2Fid")
