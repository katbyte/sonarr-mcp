...
func TestOperationPutSeriesById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutSeriesById(t.Context(), "p/id", SeriesResource{}, PutSeriesByIdOperationOptions{
		MoveFiles: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v3/series/p%2Fid")
	expectQuery(t, r, "moveFiles", "true")
